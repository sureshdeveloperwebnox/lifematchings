...
class ProfileController extends Controller
{
    ...
    {
        $astrologies = Astrology::where('user_id', auth()->id())
        ...
        if ($astrologies) {
            return (new AstronomicInformation($astrologies))->additional([
                'result' => true
            ]);
        } else {
            return $this->failure_data(null);
        }
    }
    ...
    public function public_profile($id)
    {
        ...
        if ($user) {
            ...
            $astrologies = Astrology::where('user_id', $user->id)
                ->leftJoin('sun_signs', function($join) {
                    $join->where(function($query) {
                        $query->whereColumn('astrologies.sun_sign', DB::raw('CAST(sun_signs.id AS CHAR)'))
                              ->orWhereColumn('astrologies.sun_sign', 'sun_signs.name');
            ...
            $data['astrologies'] = $astrologies ? new AstronomicInformation($astrologies) : null;
