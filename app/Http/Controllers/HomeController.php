...
class HomeController extends Controller
{
    ...
    public function dashboard()
    {
        ...
        if ($user->user_type == 'member') {
            ...
            $report = AstrologyReport::where('memberId', auth()->user()->id)->first();
            
            
            return view('frontend.member.dashboard', compact('similar_profiles', 'report', 'member'));
        } else {
            abort(404);
        }
    }
    ...
    public function member_listing(Request $request)
    {
        ...
        $sun_sign_id    = ($request->sun_sign != null) ? $request->sun_sign : null;
        $moon_sign_id   = ($request->moon_sign != null) ? $request->moon_sign : null;
        $lagnam_id      = ($request->lagnam != null) ? $request->lagnam : null;
        ...
        if (!empty($sun_sign_id)) {
            // Handle both ID (numeric) and name (string) formats for backward compatibility
            $sun_sign_obj = \App\Models\SunSign::find($sun_sign_id);
            $sun_sign_name = $sun_sign_obj ? $sun_sign_obj->name : '';
            $user_ids = \App\Models\Astrology::where(function($query) use ($sun_sign_id, $sun_sign_name) {
                $query->where('sun_sign', $sun_sign_id);
                if (!empty($sun_sign_name)) {
        ...
        if (!empty($moon_sign_id)) {
            // Handle both ID (numeric) and name (string) formats for backward compatibility
            $moon_sign_obj = \App\Models\MoonSign::find($moon_sign_id);
            $moon_sign_name = $moon_sign_obj ? $moon_sign_obj->name : '';
            $user_ids = \App\Models\Astrology::where(function($query) use ($moon_sign_id, $moon_sign_name) {
                $query->where('moon_sign', $moon_sign_id);
                if (!empty($moon_sign_name)) {
        ...
        if (!empty($lagnam_id)) {
            // Handle both ID (numeric) and name (string) formats for backward compatibility
            $lagnam_obj = \App\Models\SunSign::find($lagnam_id);
            $lagnam_name = $lagnam_obj ? $lagnam_obj->name : '';
            $user_ids = \App\Models\Astrology::where(function($query) use ($lagnam_id, $lagnam_name) {
                $query->where('lagnam', $lagnam_id);
                if (!empty($lagnam_name)) {
        ...
        return view('frontend.member.member_listing.index', compact('users', 'age_from', 'age_to', 'member_code', 'matital_status', 'religion_id', 'caste_id', 'sub_caste_id', 'mother_tongue', 'profession', 'country_id', 'state_id', 'city_id', 'min_height', 'max_height', 'member_type', 'sun_sign_id', 'moon_sign_id', 'lagnam_id'));
    }
    ...
    public function new_verify(Request $request)
    {
        $email = $request->email;
        $userId = auth()->user()->id;
        $isEmailExist = User::where('email', $email)->where('id', '!=', $userId)->first();
        if($isEmailExist){
            $response['status'] = 2;
    ...
    {
      return view('admin.members.astrologyreportpdf');
    }
}
