            ...
            @if (get_setting('member_astronomic_information_section') == 'on')
                <div class="card">
                    ...
                    <div class="card-body">
                        <table class="table">
                            <tr>
                                <th>{{ translate('Rasi/Zodiac Sign') }}</th>
                                <td>
                                    @php
                                        $sun_sign_display = '';
                                        if($member->astrologies && $member->astrologies->sun_sign) {
                                            if(is_numeric($member->astrologies->sun_sign)) {
                                                $sun_sign_obj = \App\Models\SunSign::find($member->astrologies->sun_sign);
                                                $sun_sign_display = $sun_sign_obj ? $sun_sign_obj->name : '';
                                            } else {
                                                $sun_sign_display = $member->astrologies->sun_sign;
                                            }
                                        }
                                    @endphp
                                    {{ $sun_sign_display }}
                                ...
                                <th>{{ translate('Star/Nakshatra') }}</th>
                                <td>
                                    @php
                                        $moon_sign_display = '';
                                        if($member->astrologies && $member->astrologies->moon_sign) {
                                            if(is_numeric($member->astrologies->moon_sign)) {
                                                $moon_sign_obj = \App\Models\MoonSign::find($member->astrologies->moon_sign);
                                                $moon_sign_display = $moon_sign_obj ? $moon_sign_obj->name : '';
                                            } else {
                                                $moon_sign_display = $member->astrologies->moon_sign;
                                            }
                                        }
                                    @endphp
                                    {{ $moon_sign_display }}
                             ...
                             <tr>
                                <th>{{ translate('Lagnam/Lagna') }}</th>
                                <td>
                                    @php
                                        $lagnam_display = '';
                                        if($member->astrologies && $member->astrologies->lagnam) {
                                            if(is_numeric($member->astrologies->lagnam)) {
                                                $lagnam_obj = \App\Models\SunSign::find($member->astrologies->lagnam);
                                                $lagnam_display = $lagnam_obj ? $lagnam_obj->name : '';
                                            } else {
                                                $lagnam_display = $member->astrologies->lagnam;
                                            }
                                        }
                                    @endphp
                                    {{ $lagnam_display }}
                            ...
                            <div class="row">
                                ...
                                <div class="col-6 @if(in_array($key, [0,1])) border-top @endif border-bottom" style="padding: 0.75rem;">
                                    <div class="row">
                                        ...
                                        <div class="col-6 fs-13">{{ $attribute->additional_member_info()->where('user_id', $member->id)->first()->value ?? null }}</div>
                                    </div>
                                </div>
                                @endforeach
                            </div>
