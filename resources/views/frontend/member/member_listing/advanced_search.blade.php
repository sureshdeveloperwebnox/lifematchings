            ...
            <div class="pb-4">
                <form action="{{ route('member.listing') }}" method="get">
                    ...
                    <div class="row">
                        <div class="col-lg-12">
                            <div class="form-group mb-3">
                                <label class="form-label" for="sun_sign">{{ translate('Rasi/Zodiac Sign') }}</label>
                                @php $sun_signs = \App\Models\SunSign::orderBy('name')->get(); @endphp
                                <select name="sun_sign" id="sun_sign" class="form-control aiz-selectpicker" data-live-search="true">
                                    <option value="">{{translate('Select Rasi/Zodiac Sign')}}</option>
                                    @foreach ($sun_signs as $sun_sign)
                                        <option value="{{ $sun_sign->id }}" @if(isset($sun_sign_id) && $sun_sign_id == $sun_sign->id) selected @endif>{{ $sun_sign->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                    ...
                    <div class="row">
                        <div class="col-lg-12">
                            <div class="form-group mb-3">
                                <label class="form-label" for="moon_sign">{{ translate('Star/Nakshatra') }}</label>
                                <select name="moon_sign" id="moon_sign" class="form-control aiz-selectpicker" data-live-search="true">
                    ...
                    <div class="row">
                        <div class="col-lg-12">
                            <div class="form-group mb-3">
                                <label class="form-label" for="lagnam">{{ translate('Lagnam/Lagna') }}</label>
                                @php $sun_signs = \App\Models\SunSign::orderBy('name')->get(); @endphp
                                <select name="lagnam" id="lagnam" class="form-control aiz-selectpicker" data-live-search="true">
                                    <option value="">{{translate('Select Lagnam/Lagna')}}</option>
                                    @foreach ($sun_signs as $sun_sign)
                                        <option value="{{ $sun_sign->id }}" @if(isset($lagnam_id) && $lagnam_id == $sun_sign->id) selected @endif>{{ $sun_sign->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
...
(function() {
    ...
    function initSunMoonSignFilters() {
        if (typeof jQuery === 'undefined') {
            setTimeout(initSunMoonSignFilters, 100);
            return;
        }
        ...
        $(document).ready(function() {
            ...
            @if(isset($moon_sign_id) && $moon_sign_id)
                selectedMoonSignId = {{ $moon_sign_id }};
            @endif
    ...
    if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', initSunMoonSignFilters);
    } else {
        initSunMoonSignFilters();
    }
})();
</script>
