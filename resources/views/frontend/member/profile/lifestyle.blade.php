        ...
        <form action="{{ route('lifestyles.update', $member->id) }}" method="POST">
            ...
            <div class="form-group row">
                <div class="col-md-6">
                    ...
                    @php 
                        $user_diet = !empty($member->lifestyles->diet) ? $member->lifestyles->diet : ""; 
                    @endphp
                    <select class="form-control aiz-selectpicker" name="diet" required>
                        ...
                        <option value="non_vegetarian" @if($user_diet == 'non_vegetarian') selected @endif>{{ translate('Non-Vegetarian') }}</option>
                        <option value="egg_vegetarian" @if($user_diet == 'egg_vegetarian') selected @endif>{{ translate('Egg-Vegetarian') }}</option>
                    </select>
                    @error('diet')
