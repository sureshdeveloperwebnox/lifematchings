                ...
                <div class="col-md-6">
                    ...
                    <select name="sun_sign" id="sun_sign" class="form-control aiz-selectpicker" data-live-search="true" required>
                        ...
                        @php
                            ...
                            $selected_sun_sign = $member->astrologies->sun_sign ?? "";
                            // Handle both ID (numeric) and name (string) for backward compatibility
                            $selected_sun_sign_id = null;
                            if($selected_sun_sign) {
                                if(is_numeric($selected_sun_sign)) {
                                    $selected_sun_sign_id = $selected_sun_sign;
                                } else {
                                    $selected_sun = \App\Models\SunSign::where('name', $selected_sun_sign)->first();
                ...
                <div class="col-md-6">
                    ...
                    <select name="moon_sign" id="moon_sign" class="form-control aiz-selectpicker" data-live-search="true" required>
                        ...
                        @php
                            ...
                            $selected_moon_sign = $member->astrologies->moon_sign ?? "";
                            // Handle both ID (numeric) and name (string) for backward compatibility
                            $selected_moon_sign_id = null;
                            if($selected_moon_sign) {
                                if(is_numeric($selected_moon_sign)) {
                                    $selected_moon_sign_id = $selected_moon_sign;
                                } else {
                                    $selected_moon = \App\Models\MoonSign::where('name', $selected_moon_sign)->first();
                                    $selected_moon_sign_id = $selected_moon ? $selected_moon->id : null;
                                }
                            }
                            // Get sun sign ID for initial load
                            $selected_sun_sign_id_for_moon = null;
                            // Try to get from stored sun_sign value
                            if($member->astrologies && $member->astrologies->sun_sign) {
                                if(is_numeric($member->astrologies->sun_sign)) {
                                    ...
                                    $selected_sun_sign_id_for_moon = $selected_sun ? $selected_sun->id : null;
                                }
                            }
                        @endphp
            ...
            <div class="form-group row">
                ...
                <div class="col-md-6">
                    ...
                    <select name="lagnam" id="lagnam" class="form-control aiz-selectpicker" data-live-search="true" required>
                        ...
                        @php
                            $selected_lagnam = $member->astrologies->lagnam ?? "";
                            // Handle both ID (numeric) and name (string) for backward compatibility
                            $selected_lagnam_id = null;
                            if($selected_lagnam) {
                                if(is_numeric($selected_lagnam)) {
                                    $selected_lagnam_id = $selected_lagnam;
                                } else {
                                    $selected_lagnam_obj = \App\Models\SunSign::where('name', $selected_lagnam)->first();
...
(function() {
    ...
    function initAstronomicDropdowns() {
        ...
        $(document).ready(function() {
            ...
            var selectedMoonSign = '{{ $member->astrologies->moon_sign ?? "" }}';
            var selectedMoonSignId = null;
            @if($member->astrologies && $member->astrologies->moon_sign)
                @php
                    // Check if stored value is ID (numeric) or name (string)
            ...
            setTimeout(function() {
                if (typeof $.fn.selectpicker !== 'undefined') {
                    $('#sun_sign, #moon_sign, #lagnam').selectpicker();
                    
                    // Handle sun sign change
                    $('#sun_sign').on('changed.bs.select', function(e) {
